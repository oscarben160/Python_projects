...
Processing Strings From ipod.txt (reviews for early ipod)
"""
...
with open("ipod.txt", 'r') as inFile:
  lines = inFile.readlines()
  nchar_ipod = [len(s) for s in lines] # why use "s". what is it doing?
...
index_longlines = [i for i, s in enumerate(lines) if len(s) > 100] # what i for i doing? (index for index, string in...)
...
df11 = df[(df['price'] >= 200000) | (df['price'] <= 300000)]['street'].copy() # do i need to use .copy() ??
print(df11)
